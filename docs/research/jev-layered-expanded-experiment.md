...
The focus gate did most of the filtering: 626 → 30 pairs, while keeping 22 of 27 known positive pair relationships. The overlap gate kept those 22, then the difference gate removed one positive along with two other candidates. Compared with single pass, the final layered queue lost three known positive relationships and gained one, a net loss of two. For example, it missed repeated requests about an investigation and an installation. It did identify a detail that single pass missed. Those misses are why a cleaner queue cannot by itself establish a better save outcome.
...
For new runs, live-store fingerprinting is off unless `TITAN_BENCH_VERIFY_DBS` names the database paths to inspect. Scratch-store verification still runs by default.
