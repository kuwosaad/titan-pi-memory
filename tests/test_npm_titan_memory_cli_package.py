...
PACKAGE_VERSION = json.loads((PACKAGE_DIR / "package.json").read_text(encoding="utf-8"))["version"]
...
class NpmTitanMemoryCliPackageTests(unittest.TestCase):
    ...
    def test_package_exposes_titan_bin_and_prepares_runtime(self):
        ...
        self.assertEqual(
            ...
            "node scripts/prepare-runtime.js && node scripts/audit-runtime.js && python ../../scripts/package_gate.py --pack",
        )
    ...
    def test_prepare_runtime_uses_committed_opencode_bundle(self):
        ...
        for relative_path in (
            ...
            with self.subTest(relative_path=relative_path):
                ...
                generated = PACKAGE_DIR / "runtime" / relative_path
                # This is the portable source-archive check.  It deliberately
                # asserts both inputs and generated bytes so an archive cannot
                # pass by silently omitting a required runtime surface.
                self.assertTrue(source.is_file(), f"runtime source is missing: {relative_path}")
                self.assertTrue(generated.is_file(), f"prepared runtime is missing: {relative_path}")
                self.assertEqual(generated.read_bytes(), source.read_bytes())

                # Keep the stronger committed-source assertion for an actual
                # git checkout.  Release candidates are often assembled from
                # git archives and have no .git directory to query.
                try:
                ...
                is_checkout = (
                    ...
                    and Path(checkout.stdout.strip()).resolve() == ROOT_DIR.resolve()
                )
                if is_checkout:
    ...
    def test_runtime_cache_files_do_not_trigger_reinstallation(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            version_dir = runtime_home / "versions" / PACKAGE_VERSION
    ...
    def test_marketplace_refreshes_when_plugin_script_changes(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            runtime_script = Path(tmp_dir) / "runtime" / "versions" / PACKAGE_VERSION / "tools" / "cli" / "titan.py"
