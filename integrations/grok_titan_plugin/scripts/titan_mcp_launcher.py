#!/usr/bin/env python3
"""Launch Titan MCP for the Grok plugin.

The public plugin should not silently depend on a random broken `titan` binary.
This launcher prefers a healthy local Titan CLI, falls back to the repo module,
then npm package runners when available, and prints actionable setup errors
...
def _repo_root_from_launcher() -> Path | None:
    # integrations/grok_titan_plugin/scripts/titan_mcp_launcher.py -> repo root
...
def _print_failure(diagnostics: Sequence[str], stderr: TextIO) -> None:
    ...
    print("", file=stderr)
    print("Install or repair Titan, then restart Grok:", file=stderr)
    print("  # from the Titan repo for local dogfood:", file=stderr)
    print("  pip install -e .", file=stderr)
    print("  # or install the CLI package:", file=stderr)
    print("  npm install -g titan-memory-cli", file=stderr)
    print("", file=stderr)
    print("Then verify:", file=stderr)
    print("  titan mcp --agent grok --help", file=stderr)
    print("  python3 integrations/grok_titan_plugin/scripts/titan_first_run.py", file=stderr)
