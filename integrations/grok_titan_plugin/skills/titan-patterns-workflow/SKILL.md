...
description: Use when mining, reviewing, importing, exporting, creating, accepting, or rejecting Titan learned behavior patterns from memory evidence in the Grok agent namespace.
---
...
Agent namespace is always `grok`.
...
Call via `use_tool` with qualified names like `titan-memory__patterns_status`. Pattern tools are optional on the Grok surface; if missing, say so and fall back to memory/cluster tools.
...
2. Inspect returned memory IDs and scene IDs; expand important scenes with `get_scene_context` when evidence is ambiguous.
...
Use `patterns_export_bundle` for shareable pattern bundles. By default export accepted patterns; include candidates only when the user asks.

Use `patterns_import_bundle` only with a user-provided local bundle path. Do not overwrite existing patterns unless the user explicitly asks.
