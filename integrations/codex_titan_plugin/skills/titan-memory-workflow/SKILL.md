...
1. Start with `query_memories` using the user's topic and current repo/session context. Recall searches every discovered agent namespace by default.
...
8. Writes, passive capture, traces, pending state, settings, and patterns stay in the Codex namespace. Read-only Memory and Scene recall is shared across all discovered agent namespaces.
